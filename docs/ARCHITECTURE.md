...
| 路线 | 实测表现（本机） | 结论 |
|---|---|---|
| **(a) Node 递归遍历** | **全盘实测**（2026-09-17）：757,385 文件 / 191,877 目录 / 142.81 GB → 同步单线程 **153.4 s（4,937 文件/秒）**；有界并发 32/128 → **87.8 s（8,633 文件/秒）**；96/384 并发 → **95.1 s（更慢）** | 作为 M1 主引擎；**纯 Node 在本机极限约 88 s**，且提高并发无收益 |
...
**M1 采取路线 (a) + 有界并发**。全盘实测已**推翻**早先基于 System32 的保守推算（当时"5–8 分钟"是把 System32 的 2,187 文件/秒外推到全盘所致，实际全盘达 8,633 文件/秒）：真实成本由**文件/目录数量**驱动，**与字节数无关**。本机 75.7 万文件需 **87.8 s**。

**关键结论**：纯 Node 路线在 NVMe SSD 上的**极限约 88 s，无法达到 60 s**；提高并发反而退化（96 并发 95.1 s），说明瓶颈是元数据操作本身，**只能靠 MFT 绕过**（列入 M4）。因此 M1 的性能目标已按实测**重设为可达的分层目标**（见 [ROADMAP.md](ROADMAP.md) M1 验收）。
...
3. **减少目录枚举开销（实测修正）**：早先假设"能批量拿元数据就绝不逐文件 `stat`"，**在 Windows 上不成立** —— `readdir` 不返回文件大小，硬链接去重又必须拿到 `nlink`，因此 **per-file `stat` 不可避免**。实测 `stat` 只带来 **1.6x** 减速（System32：仅 readdir 5.6 s → 加 stat 9.1 s），**目录枚举本身才是主要开销**。优化重点应放在枚举并发度与背压，而非"避免 stat"。
...
**实测修正（重要）**：早先本节声称 `C:\Windows\WinSxS`"按目录求和会重复计入多次、得出严重偏大的结果"，**实测证明该论断不成立**。WinSxS 单独测量：逻辑口径 **9.99 GB**、物理口径 **9.86 GB**，虚高仅 **1.3%**（且 9.99 GB 与 PowerShell 独立求和一致，交叉验证通过）。

真正的重复计算发生在**跨目录聚合**：`System32` 与 `WinSxS` 之间存在大量共享硬链接，两者**合并**统计时才虚高 **16.3%**（逻辑 19.41 GB → 物理 16.25 GB，虚高 3.16 GB；97,087 个文件中 40.7% 为 `nlink>1`）。

**结论与调整**：

- 实现要求不变（仍须去重、仍须双口径），但**测试对象必须选在存在跨目录共享硬链接的父目录**（如 `System32 + WinSxS` 合并，或 `C:\Windows` 整体）—— **只单测 WinSxS 会"验证通过"却测不出真正的问题**。
- **撤回对 WizTree / CCleaner 的"误报"指控**：按单目录统计它们是**正确的**，偏差源于父节点聚合口径，措辞不应夸大。
...
- **⚠️ 必须用 BigInt 读取 `ino`**：实测本机 `ino` 可达 **2.36e16**，超过 `Number.MAX_SAFE_INTEGER`（9.007e15）。用普通 `Number` 会舍入，导致**不同文件被判为同一文件而漏算**。必须使用 `fs.stat(path, { bigint: true })`；去重键建议 `ino + ':' + dev`（两者均取 BigInt）。
...
- **回收站必须走 Shell API，不能用目录遍历**：实测 `C:\$Recycle.Bin\S-1-5-18` 为 **EPERM**，普通遍历**完全看不见**回收站 —— 而其体积可观（实测 **2.28 GB / 2,140 项**），恰恰是最典型的可清理项。必须通过 Shell COM（`Shell.Application.NameSpace(10).Items()`）枚举，或提权读取。
- **Treemap 的硬链接归属策略（面积图不变式）**：共享的硬链接字节会使"子项之和 == 父项"不成立。要求把已计入其它节点的共享字节归入显式的"**共享/余量桶**"节点，保证 Treemap 面积拼块**严格守恒**，避免图形与数字对不上。
- 回收站单独成项，并给出"清空回收站"的明确入口。
...
| 现象 | 实测 |
|---|---|
| **未授权目录总数（全盘，非管理员）** | **219 个** |
| `C:\$Recycle.Bin\S-1-5-18` | **EPERM** —— 回收站 **2.28 GB / 2,140 项完全不可见**，必须走 Shell API |
| `C:\Program Files\WindowsApps` | **EPERM**（求和显示 0.00 GB，实为读不到） |
| `C:\PerfLogs` | EPERM |
| `C:\Windows\System32\drivers\DriverData` | EPERM |
| `...\Microsoft\Windows\WER\ReportArchive\AppCrash_*` | **EPERM（大量）** —— 崩溃报告，**属可清理但读不到**，是规则库盲区 |
| `C:\ProgramData\Kaspersky Lab\*`、`...\Microsoft\Crypto\*`、`...\Microsoft\Search\Data` 等 | EPERM（安全软件与系统数据目录） |
| `C:\Windows\System32`（递归） | 18 处读取错误 |

**结论**：非管理员扫描的结果天然是**下界**，且缺口集中在**最有清理价值的位置**（回收站、崩溃报告）。UI 必须常驻提示未授权目录数量，并提供提权重扫入口。

### A.5 性能基线（全盘口径）

> 测量条件：C 盘位于 **NVMe SSD**（SAMSUNG MZVLB512HBJQ-000L2），Win10 22H2，非管理员，Node v22.22.3，杀软实时防护开启。全盘规模 = 757,385 文件 / 191,877 目录 / 142.81 GB（逻辑口径）。

| 方案 | 全盘实测耗时 | 速率 | 结论 |
|---|---|---|---|
| Node 同步单线程递归 | **153.4 s** | 4,937 文件/秒 | 可用但慢 |
| **Node 有界并发 32 workers / stat 128** | **87.8 s** ✅ | **8,633 文件/秒** | **M1 采用** |
| Node 有界并发 96 / stat 384 | **95.1 s**（更慢） | 7,969 文件/秒 | **并发过高反而退化 → 瓶颈不在并发度** |
| Node 异步**无上限**并发 | 退化/停滞（早先实测） | — | 不可用；**有界并发 + 背压是正确性前提** |
| `robocopy /L /E` 枚举 | 仅测 `System32` 4.0–7.4 s | — | 输出解析脆弱、无硬链接信息 → 仅作辅助 |
| NTFS MFT 直读 | 未实测（本机无实现） | 预计秒级 | 突破 88 s 极限的唯一途径 → M4 |

**关键结论**：纯 Node 在 NVMe SSD 上的**极限约 88 s**，提高并发无收益（96 并发更慢）—— 瓶颈是元数据操作本身，**只能靠 MFT 绕过**。另注：数值含杀软实时扫描带来的波动；HDD 介质上并发收益将大幅下降，性能目标需按介质分档。

### A.6 基线快照（供 M1 回归对比）

```
C 盘              总量 150.7 GB / 已用 144.8 GB / 剩余 5.9 GB
全盘规模          757,385 文件 / 191,877 目录 / 142.81 GB（逻辑口径）
扫描耗时          同步 153.4 s ↔ 有界并发(32/128) 87.8 s ↔ 96/384 95.1 s
硬链接文件数      58,266

C:\Users\<用户名>  84.52 GB   (473,201 文件，占已用 58%)
C:\Windows        29.69 GB   (218,055 文件)
C:\Program Files  11.74 GB
C:\ProgramData     9.07 GB
Program Files x86  6.19 GB
WinSxS             9.99 GB   逻辑口径 (物理 9.86 GB，虚高 1.3%)
AppData\Local\Packages 6.22 GB
回收站             2.28 GB / 2,140 项  (需 Shell API，遍历不可见)
Windows\Installer  1.60 GB
ProgramData\Package Cache 0.45 GB
Chrome/Edge 缓存   0.28 / 0.25 GB
AppData\Local\Temp 0.16 GB
不存在             hiberfil.sys / pagefile.sys / swapfile.sys / Windows.old

安全可清理量级     约 3 GB  (回收站 2.28 + 浏览器 0.53 + Temp 0.16)
System32+WinSxS   逻辑 19.41 → 物理 16.25 GB  (跨目录去重虚高 16.3%)
未授权目录         219 个
```
...
### A.8 M1 可行性实测结论（2026-09-17 全盘）

| 结论 | 依据 |
|---|---|
| 纯 Node 路线可达 **约 88 s**（75.7 万文件），**无法达到 60 s** | 最佳配置 87.8 s；并发调高无收益（95.1 s） |
| 性能成本由**文件/目录数量**驱动，**与字节数无关** | 142.81 GB 耗时 87.8 s；同样 GB 数若是少量大文件会快得多 |
| `ino` 必须用 **BigInt** | 实测 ino = 2.36e16 > `MAX_SAFE_INTEGER` = 9.007e15 |
| per-file `stat` **不可避免** | readdir 不返回大小；去重需 nlink。实测仅带来 1.6x 减速 |
| junction 可用 `Dirent.isSymbolicLink()` 识别 | 根目录 `Documents and Settings` → `C:\Users`，跟随将虚增 84.77 GB |
| 长路径无障碍 | 589 字符路径在未开启 `LongPathsEnabled` 时正常读写 |
| 回收站**必须走 Shell API** | `$Recycle.Bin\S-1-5-18` EPERM，遍历完全不可见 |
| 安全可清理量级**仅约 3 GB** | 与"清出大空间"的直觉不符 → **价值主张必须诚实化** |

**据此产生的 M1 变更**：性能目标改为分层（首屏 / 全盘 / 缓存复用）、M1 拆为 M1a（内核 + harness 门禁）/ M1b（UI）、规则库优先级改为用户目录优先。详见 [ROADMAP.md](ROADMAP.md)。
...
| 事项 | 影响 | 决策时机 |
|---|---|---|
| 开源协议 | 仓库对外形象；**已决：MIT**（项目名已定：Cleaner-C） | ✅ 已定 |
